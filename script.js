...
// ⌨️ 키보드 꾹 누르기 조작 (부드러운 이동)
    ...
    player.currentX = Math.max(0, Math.min(canvas.width - 60, player.currentX));
        ...
        // 2. 적(👾) 생성
        if (Math.random() < 0.04) {
            shootingEnemies.push({ x: Math.random() * (canvas.width - 50), y: -50, speed: 3 + Math.random() * 2 });
        }
...
// 슈팅 모드일 때의 전용 조작
    if (gameState === 'SHOOTING') {
        ...
        // 캐릭터를 드래그하는 방향으로 이동시킵니다.
        player.currentX += xDiff * 0.5; 
        // 화면 밖으로 나가지 않게 제한
        player.currentX = Math.max(0, Math.min(canvas.width - 60, player.currentX));
        return;
    }
...
/* --- [추가] 모바일 드래그(손가락 따라오기) 조작 --- */
window.addEventListener('touchmove', e => {
    // 1. 오직 슈팅 보너스 모드일 때만 이 조작이 작동합니다.
    if (gameState === 'SHOOTING') {
        // 2. 현재 내 손가락이 터치하고 있는 가로(X) 좌표를 가져옵니다.
        const touchX = e.touches[0].clientX;
        
        // 3. 게임 화면(캔버스)의 왼쪽 시작 지점을 계산해서 뺍니다.
        const canvasRect = canvas.getBoundingClientRect();
        const relativeX = touchX - canvasRect.left;
        
        // 4. 캐릭터의 중심(30px)이 내 손가락 끝에 오도록 위치를 정합니다.
        player.currentX = relativeX - 30;
        
        // 5. 캐릭터가 화면 왼쪽이나 오른쪽 밖으로 나가지 못하게 막습니다.
        player.currentX = Math.max(0, Math.min(canvas.width - 60, player.currentX));
        
        // 6. 드래그하는 동안 스마트폰 화면이 위아래로 움직이지 않게 고정합니다.
